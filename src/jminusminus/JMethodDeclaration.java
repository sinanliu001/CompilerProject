...
class JMethodDeclaration extends JAST implements JMember {
    ...
    /**
     * Method handler (computed in preAnalyze()).
     */
    protected ArrayList<String> handler;
    ...
    public void preAnalyze(Context context, CLEmitter partial) {
        ...
        if (exceptions!=null){
            ...
            for (TypeName Name : exceptions) {
                String javaname = "java/lang/";
                javaname += Name.toString();
        ...
        if (exceptions != null) {
            ...
            for (TypeName exception : exceptions) {
                handler.add("java/lang/IllegalArgumentException");
            }}*/
    ...
    public JAST analyze(Context context) {
        ...
        // method call.
        for (JFormalParameter param : params) {
            if (param.type().resolve(context) == Type.DOUBLE||param.type().resolve(context) == Type.LONG){
                int x = this.context.nextOffset();
                LocalVariableDefn defn = new LocalVariableDefn(param.type(), x);
                this.context.nextOffset();
                defn.initialize();
    ...
    public void partialCodegen(Context context, CLEmitter partial) {
        partial.addMethod(mods, name, descriptor, handler, true);
    ...
    public void codegen(CLEmitter output) {
        ...
        output.addMethod(mods, name, descriptor, handler, false);
        if (body != null) {
            body.codegen(output);
        }
        if (returnType == Type.VOID) {
