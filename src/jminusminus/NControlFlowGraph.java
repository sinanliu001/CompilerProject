...
class NBasicBlock {
    ...
    /**
     * The state array for this block that maps local variable index to the HIR instruction that
     * last affected it.
     */
    public int[] locals;
    ...
    public NBasicBlock(NControlFlowGraph cfg, int id) {
        this.cfg = cfg;
        this.id = id;
        tuples = new ArrayList<NTuple>();
        predecessors = new ArrayList<NBasicBlock>();
        successors = new ArrayList<NBasicBlock>();
        hir = new ArrayList<Integer>();
        lir = new ArrayList<NLIRInstruction>();
        isLoopHead = false;
    }
     ...
     *
     * @return the instruction identifier for the first LIR instruction, or -1.
     */
    public int getFirstLIRInstId() {
        return lir.isEmpty() ? -1 : lir.get(0).id;
    }

    /**
     * Returns the instruction identifier for the last LIR instruction, or -1.
     *
     * @return the instruction identifier for the last LIR instruction, or -1.
     */
    public int getLastLIRInstId() {
        return lir.isEmpty() ? -1 : lir.get(lir.size() - 1).id;
    }
     ...
     * @return the LIR instruction with the given id, or null.
     */
    public NLIRInstruction getInstruction(int id) {
        for (NLIRInstruction inst : this.lir) {
            if (inst.id == id) {
                return inst;
            }
        }
     ...
     * @return true if there is no LIR instruction with the given id, and false otherwise.
     */
    public boolean idIsFree(int id) {
        return getInstruction(id) == null;
    }

    /**
     * Inserts the given LIR instruction at the appropriate place in this block's lir array based
     * on its id -- preserving order by id.
     *
     * @param inst the LIR instruction to be inserted.
     */
...
class NControlFlowGraph {
    ...
    /**
     * The total number of intervals. This is used to name split children and grows as more
     * intervals are created by spills.
     */
    ...
    public NControlFlowGraph(CLConstantPool cp, CLMethodInfo m) {
        ...
        // Connect up the blocks for this method, ie, build its control flow graph.
        ...
        for (int j = 0; j < basicBlocks.size(); j++) {
            ...
            switch (tuple.opcode) {
                ...
                case JSR_W:
                    ...
                    offset = intValue(operandByte1, operandByte2, operandByte3, operandByte4);
                    target = blockAt[tuple.pc + offset];
                    block.successors.add(target);
                    target.predecessors.add(block);
                    break;
                ...
                case ATHROW:
                    break;
                case TABLESWITCH: // Unsupported
                    break;
                case LOOKUPSWITCH: // Unsupported
                    break;
                default:
                    if (j < basicBlocks.size() - 1) {
                        block.successors.add(basicBlocks.get(j + 1));
        ...
        // Calculate the ref count and number of forward branches to each block in the this cfg.
        for (NBasicBlock block : basicBlocks) {
    ...
    /**
     * Implements loop detection algorithm to figure out if the specified block is a loop head or
     * a loop tail. Also calculates the number of backward branches to the block.
     *
     * @param block a block.
    ...
    /**
     * Removes blocks that cannot be reached from the begin block (B0). Also removes these blocks
     * from the predecessor lists.
     */
    ...
    /**
     * Computes the dominator of each block in this cfg recursively given the starting block and
     * its predecessor.
     *
            ...
            if (block.predecessors.size() == 1) {
                ...
                if (block.isLoopHead) {
                    ...
                    for (int i = 0; i < block.locals.length; i++) {
                        ...
                        phi.getArguments().add(block.locals[i]);
                        for (int j = 1; j < block.predecessors.size(); j++) {
                            phi.getArguments().add(phi.id);
                        }
            ...
            for (NTuple tuple : block.tuples) {
                ...
                switch (insInfo.opcode) {
                    case MULTIANEWARRAY: {
                        ...
                        String type = new String(((CLConstantUtf8Info) cp.cpItem(classIndex)).b);
                    ...
                    case AALOAD: {
                        ...
                        NHIRInstruction ins2 = new NHIRArithmetic(block, hirId++, IADD, operand1,
                                ins1.id);
                        ...
                        NHIRInstruction ins4 = new NHIRArithmetic(block, hirId++, IMUL, operand2,
                                ins3.id);
                        ...
                        ins = new NHIRALoad(block, hirId++, insInfo.opcode, ins2.id, ins4.id,
                                "L", "L");
                        block.cfg.hirMap.put(ins.id, ins);
                        block.hir.add(ins.id);
                        operandStack.push(ins.id);
                    ...
                    case IASTORE: {
                        ...
                        ins = new NHIRAStore(block, hirId++, insInfo.opcode, ins2.id, ins4.id,
                                operand3, "I", "I");
                        block.cfg.hirMap.put(ins.id, ins);
                        block.hir.add(ins.id);
                        break;
                    ...
                    case LDC: {
                        ...
                        // Only allowing ldc of string constants for now.
                        int stringIndex =
                                ((CLConstantStringInfo) cp.cpItem(operandByte1)).stringIndex;
                        String s = new String(((CLConstantUtf8Info) cp.cpItem(stringIndex)).b);
                    ...
                    case IF_ICMPLE: {
                        ...
                        NBasicBlock trueDestination = pcToBasicBlock.get(tuple.pc + offset);
                        NBasicBlock falseDestination = pcToBasicBlock.get(tuple.pc + 3);
                        ins = new NHIRConditionalJump(block, hirId++, lhs, rhs, insInfo.opcode,
                    ...
                    case PUTSTATIC: {
                        ...
                        int nameAndTypeIndex =
                                ((CLConstantFieldRefInfo) cp.cpItem(index)).nameAndTypeIndex;
                        int nameIndex = ((CLConstantClassInfo) cp.cpItem(classIndex)).nameIndex;
                        String target = new String(((CLConstantUtf8Info) cp.cpItem(nameIndex)).b);
                        int fieldNameIndex =
                                ((CLConstantNameAndTypeInfo) cp.cpItem(nameAndTypeIndex)).nameIndex;
                        int fieldDescIndex =
                                ((CLConstantNameAndTypeInfo) cp.cpItem(nameAndTypeIndex)).descriptorIndex;
                        String name =
                                new String(((CLConstantUtf8Info) cp.cpItem(fieldNameIndex)).b);
                        String desc =
                                new String(((CLConstantUtf8Info) cp.cpItem(fieldDescIndex)).b);
                        if (insInfo.opcode == PUTSTATIC) {
                            ins = new NHIRPutField(block, hirId++, insInfo.opcode, target, name,
                                    shortType(desc), desc, operandStack.pop());
                        } else {
                            ins = new NHIRGetField(block, hirId++, insInfo.opcode, target, name,
                                    shortType(desc), desc);
                            operandStack.push(ins.id);
                        }
                        block.cfg.hirMap.put(ins.id, ins);
                    ...
                    case INVOKESTATIC: {
                        ...
                        operandByte2 = tuple.operands.get(1);
                        int index = shortValue(operandByte1, operandByte2);
                        int classIndex = ((CLConstantMethodRefInfo) cp.cpItem(index)).classIndex;
                        int nameAndTypeIndex =
                                ((CLConstantMethodRefInfo) cp.cpItem(index)).nameAndTypeIndex;
                        int nameIndex =
                                ((CLConstantClassInfo) cp.cpItem(classIndex)).nameIndex;
                        String target = new String(((CLConstantUtf8Info) cp.cpItem(nameIndex)).b);
                        int methodNameIndex =
                                ((CLConstantNameAndTypeInfo) cp.cpItem(nameAndTypeIndex)).nameIndex;
                        int methodDescIndex = ((CLConstantNameAndTypeInfo)
                                cp.cpItem(nameAndTypeIndex)).descriptorIndex;
                        String name =
                                new String(((CLConstantUtf8Info) cp.cpItem(methodNameIndex)).b);
                        String desc =
                                new String(((CLConstantUtf8Info) cp.cpItem(methodDescIndex)).b);
                        ArrayList<Integer> args = new ArrayList<Integer>();
                        ...
                        ins = new NHIRInvoke(block, hirId++, insInfo.opcode, target, name, args,
                                shortType(returnType), returnType);
                    ...
                    case IRETURN:
                    case ARETURN: {
                        ins = new NHIRReturn(block, hirId++, insInfo.opcode, operandStack.pop());
                        block.cfg.hirMap.put(ins.id, ins);
                        block.hir.add(ins.id);
                        break;
                    }
    ...
    public void eliminateRedundantPhiFunctions() {
        for (int ins : hirMap.keySet()) {
            ...
            if (hir instanceof NHIRPhiFunction) {
                ...
                int firstArg = phi.getArguments().get(0);
                boolean match = true;
                NBasicBlock block = phi.block;
                if (!block.isLoopHead) {
                    continue;
                }
                for (int i = 1; i < phi.getArguments().size(); i++) {
                    if (phi.getArguments().get(i) != block.predecessors.get(i).locals[phi.getLocal()]) {
                        match = false;
                        phi.getArguments().set(i, block.predecessors.get(i).locals[phi.getLocal()]);
                    }
                }
    ...
    public void hirToLir() {
        ...
        // We now know how many virtual registers are needed, so we can initialize bitset fields
        // in each block that are needed for interval calculation.
    ...
    /**
     * Resolves the phi functions in this cfg, i.e., for each x = phi(x1, x2, ..., xn) generate
     * an (LIR) move xi, x instruction at the end of the predecessor i of thte block defining the
     * phi function; if the instruction there is a branch, add the instruction prior to the branch.
     */
        ...
        for (int ins1 : hirMap.keySet()) {
            ...
            if (hir instanceof NHIRPhiFunction) {
                ...
                for (int i = 0; i < phi.getArguments().size(); i++) {
                    NHIRInstruction arg = hirMap.get(phi.getArguments().get(i));
                    if (arg.sType.equals("")) {
                        continue;
                    }
     ...
     * @return the basic block at a particular instruction id, or null.
     */
    ...
    public void renumberLirInstructions() {
        ...
        for (NBasicBlock block : basicBlocks) {
            ...
            for (NLIRInstruction lir : block.lir) {
                if (lir instanceof NLIRLoadLocal && ((NLIRLoadLocal) lir).getLocal() < 4) {
    ...
    /**
     * Replaces references to virtual registers in LIR instructions with references to physical
     * registers.
     */
    ...
    // Returns true if instr is a (conditional or unconditional) jump, and false otherwise.
    private boolean isHIRJmp(NHIRInstruction instr) {
        return (instr instanceof NHIRGoto || instr instanceof NHIRConditionalJump);
    }

    // Clears the visitation information in each block in this cfg.
    ...
    // Returns the common dominator of the given basic block and its predecessor.
    ...
    // Merges the locals from each of the predecessors of the specified block with the locals in
    // the block.
    ...
    // Merges the locals in block b with the locals in block a.
    private void mergeLocals(NBasicBlock a, NBasicBlock b) {
        for (int i = 0; i < a.locals.length; i++) {
            if (a.cfg.hirMap.get(a.locals[i]).equals(b.cfg.hirMap.get(b.locals[i]))) {
                ...
                args.add(a.locals[i]);
                args.add(b.locals[i]);
                NHIRInstruction ins = new NHIRPhiFunction(a, NControlFlowGraph.hirId++, args, i);
                a.locals[i] = ins.id;
                a.hir.add(ins.id);
    ...
    // Converts the bytecode in the specified list to their tuple representations.
    ...
    // Constructs and returns a short integer from two unsigned bytes specified.
    private short shortValue(short a, short b) {
        return (short) ((a << 8) | b);
    }

    // Constructs and returns an integer from the four unsigned bytes specified.
    ...
    // Returns the short form of the specified type descriptor.
    ...
    // Returns the argument count (number of formal parameters) for the method with the given
    // descriptor.
    ...
    // Returns a list containing the argument types for the method with the given descriptor.
